...
import {
  ...
  TocPopoverTrigger,
  TocPopoverContent,
} from "@/components/notebook/page-client";
import {
  Toc,
  TOCItems,
  type TOCProps,
  TOCScrollArea,
} from "@/components/notebook/layout/toc";
import { buttonVariants } from "@/components/ui/button";
import { Edit, Text } from "lucide-react";
import { I18nLabel } from "fumadocs-ui/contexts/i18n";
import ClerkTOCItems from "@/components/notebook/layout/toc-clerk";
...
export const DocsTitle = forwardRef<
  HTMLHeadingElement,
  HTMLAttributes<HTMLHeadingElement> & { showSoonBadge?: boolean }
>((props, ref) => {
  const { showSoonBadge, ...rest } = props;
  ...
  return (
    ...
    >
      {rest.children}
      {showSoonBadge && (
        <span className="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold bg-fd-accent text-fd-accent-foreground">
          Soon
        </span>
      )}
    </h1>
