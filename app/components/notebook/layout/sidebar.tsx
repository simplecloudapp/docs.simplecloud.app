...
const itemVariants = cva(
  "relative flex flex-row items-center gap-4 rounded-md p-2 text-start text-fd-muted-foreground [overflow-wrap:anywhere] md:py-1.5 [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      active: {
        true: "bg-primary/10 text-primary",
        false:
          "transition-colors hover:bg-sidebar-border/50 hover:text-fd-accent-foreground/80 hover:transition-none",
      },
    },
  }
);
  ...
  return (
    <Context.Provider value={context}>
      <Base.SidebarList
        ...
        {...props}
        className={cn(
          "fixed pt-[calc(var(--fd-banner-height)+var(--fd-nav-height))] bg-fd-card text-sm md:sticky top-0 md:h-(--fd-sidebar-height) z-10",
...
export function SidebarViewport(props: ScrollAreaProps) {
  return (
    <ScrollArea {...props} className={cn(props.className)}>
      <ScrollViewport
        className="py-4 pr-4"
        style={{
