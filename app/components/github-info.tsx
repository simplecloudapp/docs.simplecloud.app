...
export function GithubInfo({
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: () => getRepoStarsAndForks(owner, repo, token),
  ...
  if (isLoading) {
    return (
      <div
        className={cn(
          "flex flex-col gap-1.5 p-2 rounded-lg text-sm text-fd-foreground/80",
          props.className
        )}
      >
        <p className="animate-pulse bg-fd-accent h-4 w-32 rounded" />
      </div>
    );
  }
  ...
  const humanizedStars = humanizeNumber(data!.stargazers_count);
