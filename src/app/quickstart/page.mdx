...
This guide will help you set up a basic SimpleCloud installation using the CLI, including a proxy and an initial server group functioning as a lobby. We'll cover the [CLI](/cli) and standardized official [Droplets](/droplets).  {{ className: 'lead' }}
...
For a functioning Minecraft server network you first need to create a [**Proxy Group**](/server-group):
...
To run an actual Minecraft server, you also need to create a [**Server Group**](/server-group):
...
After setting up the groups, configure the [Connection Plugin](/plugin/server-connection) to route players to the lobby group when joining a proxy or switching servers. Follow these steps:

1. **Download and Install the Plugin**: Get the plugin from the [releases page](https://github.com/theSimpleCloud/server-connection-plugin/releases) and place it in your proxy template's plugins folder. It's installed by default in the bundle.
