...
4. Set a joinstate for your group (e.g. `maintenance`)

## Commands

| **Command**               | **Description**                                                                                    |
|------------------------------|----------------------------------------------------------------------------------------------------|
| `joinstate group <group> <join-state>`            | Set the joinstate for a group. |
| `joinstate server <group> <numericalId> <join-state>`          | Set the joinstate for a server. |
...
#### Configuration for static tablist
```yaml
groups:
-   group-or-service: '*'
    tab-lists:
    -   header:
        ...
        - ''
tab-list-update-time: 0 #If set to 0, the tablist will not update, otherwise it will update every x milliseconds
```

#### Configuration for dynamic tablist
```yaml
groups:
-   group-or-service: '*'
    tab-lists:
...
You can use joinstates to define which user groups are allowed to join the network. You can create or adjust the joinstates in the `joinstate.yml` file.
Depending on which joinstate is currently active for a group, the corresponding authorizations that you have defined in the configuration are applied.
You can define different joinstates for each server or group. The plugin supports MiniMessage formatting for the display of rich text content.
