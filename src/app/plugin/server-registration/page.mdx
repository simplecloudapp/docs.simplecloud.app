...
<Note>
    This plugin internally uses the [server API](/api#server) to get the online cloud servers. It also provides code to make [own implementations](#own-implementation) for other proxy software easy.
</Note>
...
interface ServerRegisterer {
    ...
    // Is called whenever a server is not anymore mounted to its port
    fun unregister(server: Server)
}
```

<DangerNote>
    This is only available for Java and Kotlin projects
</DangerNote>
<DangerNote>
