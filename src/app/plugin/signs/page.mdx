...
A list of all available Rule operations:

| **Name**         | **Description**                                                                              |
|------------------|----------------------------------------------------------------------------------------------|
| **REGEX**        | Matches values using a regular expression. Flexible for complex patterns and validations.    |
| **PATTERN**      | Matches values based on predefined patterns. Suitable for reusable and standardized formats. |
| **EQUALS**       | Checks for exact equality between two values.                                                |
| **CONTAINS**     | Verifies if a value contains a specific substring.                                           |
| **STARTS_WITH**  | Ensures the value begins with the specified prefix.                                          |
| **ENDS_WITH**    | Ensures the value ends with the specified suffix.                                            |
| **GREATER_THAN** | Compares numerical values to check if one is greater than the other.                         |

The matcher configuration includes a `negate` option, which is set to `false` by default. When set to `true`, any of the operations will perform the inverse of their standard behavior.
...
priority: 1 # Optional: Determines the order in which layouts are checked (Defaults to '0')
            ...
            # If multiple layouts match a rule, the one with
...
frames: # Required: List of frames that will be displayed in sequence
  - sides: 'FRONT' # Optional: Determines on what side this frame should be displayed (Defaults to 'FRONT') ('FRONT', 'BACK', 'BACK')
