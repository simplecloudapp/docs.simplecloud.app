...
<CodeGroup>
    ...
    ```kotlin {{ title: 'Kotlin' }}
    val server = api.getServers().getServerById("lobby-1")
    ```

    ```java {{ title: 'Java' }}
    api.getServers().getServerById("lobby-1").thenAccept(server -> {
        // Handle server
    });
    ```
...
<CodeGroup>
    ...
    ```java {{ title: 'Java' }}
    api.getServers().updateServerState("lobby-1", ServerState.AVAILABLE).thenAccept(server -> {
        // State updated
    });
    ```
...
<CodeGroup>
    ...
    ```java {{ title: 'Java' }}
    // Stop by ID
    api.getServers().stopServer("lobby-1", ServerStopCause.API_STOP).thenAccept(server -> {
        // Server stopped
    });
...
| Property | Type | Description |
|----------|------|-------------|
| uniqueId | `string` | Unique identifier (e.g., "lobby-1") |
| type | `enum` | Server type (UNKNOWN_SERVER, SERVER, PROXY) |
