...
- `-s, --server-url string`: Update the server url. Can be:
  ...
  - `update`: Update to the latest URL for the current version from our [manifest](https://github.com/theSimpleCloud/simplecloud-manifest) (e.g., getting the newest Paper build for 1.21.4, but will not update to 1.21.5). The [manifest](https://github.com/theSimpleCloud/simplecloud-manifest) updates all software URLs once per day.
...
```bash
simplecloud stop cloud [flags]
```

**Flags**

- `-h, --help`: Help for cloud
- `-s, --servers`: Stop with all servers
...
```bash
simplecloud stop server <name> [flags]
```
