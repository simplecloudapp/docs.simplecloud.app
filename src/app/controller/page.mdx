...
The structure of the controller is very simple. It checks every few seconds if there are any updates on servers or server groups. If so, it will try to talk to a [server-host](/droplet/serverhost) and apply those changes. The contacted [server-host](/droplet/serverhost) will do the rest of the work, like starting, stopping, or updating a server.
