...
The controller is pretty easy to understand. It's just a small program that keeps track of [server groups](/resources/server-group) and their online [servers](/resources/server) and manages them. It's the heart of SimpleCloud v3. {{ className: 'lead' }}

## Understanding the controller

The structure of the controller is very simple. It checks every few seconds if there are any updates on servers or server groups. If so, it will try to talk to a [server-host](/droplet/serverhost) and apply those changes. For example, if additional servers are needed or group configurations change, this is forwarded to the right server host. The contacted [server-host](/droplet/serverhost) will do the rest of the work, like starting, stopping, or updating a server.
