...
The controller is pretty easy to understand. It's just a small program, that keeps track of [server groups](/server-group) and their online [servers](/server) and manages them. It is the heart of SimpleCloud v3. {{ className: 'lead' }}
...
When you install SimpleCloud, the controller is automatically installed. You can install the controller manually by running the [CLI](/cli) command below, but it's not recommended. Let SimpleCloud handle the installation of the controller.
...
Yes, you can run the controller in Docker. We provide a Docker image for the controller you can use. You can find a guide on how to run SimpleCloud in Docker [here](#).

If you want every server to run in Docker, you can use the [server-host-docker](/droplet/server-host-docker) droplet.
...
This also means that you can restart the controller at any time. The controller will automatically detect all running servers and server groups and update the database accordingly.
...
We would say no. The controller is a lot more lightweight and easier to manage than the old v2 Manager. It is just a small program that runs in the background and manages all your servers and server groups. The old v2 Manager was a big program that was responsible for managing servers, server groups, templates, modules and so on.

But like the old v2 Manager, the controller is the heart of SimpleCloud. Without it, SimpleCloud would not work, and momentarily there is no way to run multiple controllers at the same time.

## Does the controller start droplets?

No, the controller does not start droplets, but the [CLI](/cli) does. The controller is just responsible for managing servers and server groups.