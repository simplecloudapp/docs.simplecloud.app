...
# Server Host Droplet
The serverhost droplet is one of our first and most important droplets. It is responsible for configuring and managing minecraft servers. {{ className: 'lead' }}

## Understanding the serverhost droplet
It's not that hard to understand the serverhost droplet. It is a simple droplet that is responsible for starting and managing minecraft servers.
...
<Note>
    If you're looking for a dockerized version of the server host droplet, we're currently working on it. It will be available soon.
</Note>

## Multi-Root installation
The serverhost droplet is designed to be installed on multiple machines. This is called a multi-root installation.
This means that you can have multiple serverhost droplets running on different machines, all connected to our [controller](/controller).

To set up a multi-root installation you need to install our [CLI](/cli#installation) on the machine you want to install the serverhost droplet on.
...
<Note>
    Make sure to set content of the `secrets/auth.secret` file to the same value as the `auth.secret` in the controller, otherwise the serverhost droplet won't be able to connect to the [controller](/controller#authentication).
</Note>
...
<Note>
    You have to make sure that the proxies send the correct player count, if you're not using our [proxy info plugin](/plugin/proxy), otherwise the player count will be wrong.
</Note>

## Needed group properties
The serverhost droplet needs some [dynamic properties](/server-group#dynamic-properties) to work. For a group to start,
...
If the serverhost droplet crashes, you have to restart it manually. What's really cool is that the managed Minecraft servers will continue to run, even if the serverhost droplet is not running.
