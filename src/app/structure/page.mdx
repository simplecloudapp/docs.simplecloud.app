...
to start servers, so for it to do something you need a [Server Host Droplet](/droplet/serverhost) (formerly known as Wrapper). This droplets main objective is to actually start Servers, therefore templates are also managed by this droplet.
This means you can easily implement [Multi-Root](/droplet/serverhost#multi-root) by just starting up a Server Host Droplet on each root server.

<Note>
    Before you read further, please first understand how [Controller](/controller) and [Server Host Droplet](/droplet/serverhost) operate.
</Note>
...
To achieve this file structure, start up the controller [jar](https://github.com/thesimplecloud/simplecloud-controller/releases/latest) in your installation dir.
...
This will generate all needed files. Now stop the controller using `CTRL+C`, and drop the Server Host Droplet [jar](https://github.com/thesimplecloud/serverhost-droplet/releases/latest) into `droplets/serverhost-droplet`.
