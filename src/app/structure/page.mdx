...
<img
    src={"https://github.com/theSimpleCloud/branding/blob/main/docs/images/structure.png?raw=true"}
    alt=""
...
The v3 is structured into components, which are the building blocks of the system and can be compared to [dropelts](/droplet). The ony difference is that components also include the [Controller](/controller), Envoy (necessary for the Dashboard), the auto updater and the [CLI](/cli).

We could've also used [dropelts](/droplet) as components, but we decided to go with a more generic name. This allows us to also have a general name for parts of the system that are not directly [droplets](/droplet).
...
We use Lua scripts for component management, providing a cross-platform solution that works on both Unix-like systems and Windows. The scripts are located in the `components/scripts` directory and are organized into components and libraries.

### Script Structure

The scripts are organized into two main directories:
- **components/**: Contains individual component scripts
