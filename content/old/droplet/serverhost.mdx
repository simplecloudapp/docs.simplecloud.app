...
This means that you can have multiple server host droplets running on different machines, all connected to our [controller](/docs/manual/controller).

To set up a multi-root installation you need to install our [CLI](/docs/manual/cli#installation) on the machine you want to install the server host droplet on.
...
<Callout>
  ...
  [controller](/docs/manual/controller#authentication).
</Callout>

## Docker Support

We are currently working on dockerizing Minecraft servers with the server host droplet. This will be possible through [actions](/docs/manual/template#actions).
...
<Callout>
  ...
  you're not using our [Proxy Essentials plugin](/docs/manual/plugin/proxy),
  otherwise the player count will be wrong.
</Callout>

## Needed Group Properties

The server host droplet needs some [dynamic properties](/docs/manual/resources/server-group#dynamic-properties) to work. For a group to start,
...
| `forwarding-secret`   | used in the velocity [configurator](#server-versions) | No            | secure secret set by the [controller](/docs/manual/controller) |
...
if the needed [dynamic group properties](/docs/manual/resources/server-group#dynamic-properties) are not set in the group configuration, a server of this
...
user specified [options](#server-options). You can set the server URL through the `server-url` [dynamic group property](/docs/manual/resources/server-group#dynamic-properties).
...
When defining a Configurator, you can use placeholders for any [controller](/docs/manual/resources/server-group#controller-properties) or [dynamic](/docs/manual/resources/server-group#dynamic-properties) property using this syntax: `%property-name%`.
...
To assign a configurator to a server, you need to set the [dynamic group property](/docs/manual/resources/server-group#dynamic-properties) `configurator` to the configurator id.
...
You can also use any [group property](/docs/manual/resources/server-group#controller-properties). Property placeholders follow this syntax:
`%PROPERTY_NAME%`
