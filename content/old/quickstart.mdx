...
SimpleCloud now offers an extensive [CLI](/docs/manual/cli) that covers everything you need to manage SimpleCloud. It's a small application that serves as an interface for sending commands to your SimpleCloud instance and is the easiest way to get started with SimpleCloud.
...
<Callout>
  If you already have a running SimpleCloud instance and want to add another Root Server, consider
  using the [Server Host Droplet](/docs/manual/droplet/serverhost).
</Callout>
...
<Callout>
  You can only start the controller locally. Remote access via the CLI is only possible once the
...
- Learn more about [Server Groups](/docs/manual/resources/groups)
- Configure [Templates](/docs/manual/resources/templates)
- Add [Droplets](/docs/manual/droplet/overview) to extend functionality
- Install [Plugins](/docs/manual/plugin/overview) for additional features
