...
This means that you can have multiple Serverhosts running on different machines, all connected to our [controller](/docs/concepts/controller).

To set up a multi-root installation you need to install our [CLI](/docs/concepts/cli#installation) on the machine you want to install the Serverhost on.
...
<Callout>
  ...
  to connect to the [controller](/docs/concepts/controller#authentication).
</Callout>

## Docker Support

We are currently working on dockerizing Minecraft servers with the Serverhost. This will be possible through [actions](/docs/manual/template#actions).
...
The Serverhost needs some [dynamic properties](/docs/manual/resources/server-group#dynamic-properties) to work. For a group to start,
...
| `forwarding-secret`   | used in the velocity [configurator](#server-versions) | No            | secure secret set by the [controller](/docs/concepts/controller) |
