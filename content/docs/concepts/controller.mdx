...
The structure of the controller is very simple. It checks every few seconds if there are any updates on servers or server groups. If so, it will try to talk to a [serverhost](/docs/manual/concepts/serverhost) and apply those changes. For example, if additional servers are needed or group configurations change, this is forwarded to the right server host. The contacted [serverhost](/docs/manual/concepts/serverhost) will do the rest of the work, like starting, stopping, or updating a server.

For example, if you want to start a server, the controller will send a message to a [serverhost](/docs/manual/concepts/serverhost) and tell it to start a server. The [serverhost](/docs/manual/concepts/serverhost) will then start the server and tell the controller that the server is now running. At last, the controller updates the status of the server in the database.
...
When you install SimpleCloud, the controller is automatically installed. If you want, you can install the controller manually by running the [CLI](/docs/concepts/cli) command below, but it's not recommended. Let SimpleCloud handle the installation of the controller.
